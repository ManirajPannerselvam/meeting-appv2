...
export async function initDB() {
    ...
    try {
        // FIX: v2 plugin-sql uses default export
...
export async function getMeeting(id: number) { // <-- ADDED
    const database = await initDB();
    if (!database) return null;
    try {
        const rows = await database.select(`SELECT * FROM meetings WHERE id=?`, [id]);
        return rows[0] || null;
    } catch (e) {
        console.error("[DB] getMeeting failed:", e);
        return null;
    }
}
...
export async function deleteMeeting(id: number) { // <-- ADDED
    const database = await initDB();
    if (!database) return false;
    try {
        await database.execute(`DELETE FROM meetings WHERE id=?`, [id]);
        window.dispatchEvent(new CustomEvent("meetings:updated"));
        return true;
    } catch (e) {
        console.error("[DB] deleteMeeting failed:", e);
        return false;
    }
}
