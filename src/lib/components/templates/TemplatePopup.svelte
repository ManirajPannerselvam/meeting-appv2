<script lang="ts">
    ...
    let shareUserId="";
    let currentUserId="8d4f3f2c-1e47-499f-b048-5e51ae34666b";
    let currentUserName="Maniraj"; 
    let currentEmail="maniraj992@gmail.com"; 
    ...
    function calcAllFormulas(){ if(!useTemplate) return; let fields=useTemplate.fields||useTemplate.data?.fields||[]; for(let f of fields){ if(f.type==='formula'&&f.formula){ let expr=String(f.formula).slice(0,MAX_FORMULA_LEN); for(let rf of fields){ let key=rf.field_name||rf.name; if(!key) continue; let raw=useFormData[key]; let val=parseFloat(raw); if(isNaN(val)) val=0; expr=expr.split(`{${key}}`).join(val.toString()); } const res=safeEval(expr); if(res!==null){ let k=f.field_name||f.name; useFormData[k]=res; } } } }
...
.name-row{ display:flex; gap:4px; align-items:center; flex-wrap:wrap; }.name-row h3{ margin:0; font-size:13px; color:#111827; }.theme-tag{ padding:1px 5px; border-radius:4px; font-size:8px; font-weight:700; color:#fff; }
...
.share-title{ margin:0; font-size:14px; color:#0f172a!important; font-weight:800!important; }
.share-sub{ color:#334155!important; font-weight:600!important; font-size:11px!important; }
