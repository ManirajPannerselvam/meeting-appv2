<script lang="ts">
  ...
  let station = $derived(data?.values?.station || '-');
  let shift = $derived(data?.values?.shift || '-');
  let input = $derived(data?.values?.input01 || 0);
  let output = $derived(data?.values?.output01 || 0);
  let remark = $derived(data?.values?.remark01 || '-');
  let yieldVal = $derived(input > 0 ? ((output / input) * 100).toFixed(2) + '%' : '-');
  
  // TRACKING - WHO SENT
  let sentBy = $derived(data?.user_name || data?.created_by_name || data?.profiles?.name || data?.sender_name || '-');
  let sentEmail = $derived(data?.user_email || data?.profiles?.email || data?.created_by_email || '');
  let sentAt = $derived(data?.created_at || data?.inserted_at || '');
  let sentUserId = $derived(data?.user_id || data?.created_by || '');
  ...
  <div class="modal">
    ...
    <div class="body">
      ...
      <div class="grid">
        ...
        <div class="item"><span>Picked By:</span><b>{sentBy}</b></div>
      </div>
...
.modal{ background:white; border-radius:12px; width:100%; max-width:440px; overflow:hidden; box-shadow:0 10px 30px rgba(0,0,0,0.2); }
.head{ display:flex; justify-content:space-between; align-items:center; padding:14px 18px; border-bottom:1px solid #e2e8f0; }
...
.avatar{ width:36px; height:36px; background:#e2e8f0; border-radius:50%; display:flex; align-items:center; justify-content:center; }
.t-info{ display:flex; flex-direction:column; flex:1; }
.t-info strong{ font-size:13px; }
.t-info span{ font-size:11px; color:#64748b; }
.t-info .time{ color:#0f172a; font-weight:600; }
.badge{ background:#111827; color:white; padding:4px 10px; border-radius:20px; font-size:11px; font-weight:700; }
.grid{ display:grid; grid-template-columns:1fr 1fr; gap:10px; }
.item{ background:#f8fafc; padding:10px 12px; border-radius:8px; display:flex; flex-direction:column; gap:3px; }
...
.remark p{ margin:6px 0 0; font-size:13px; color:#334155; }
