<script lang="ts">
    ...
    function send() {
        ...
        dispatch("sendMessage", { content: msg, files: selectedFiles });
        text = "";
        selectedFiles = [];
    ...
    $: recordingMinutes = Math.floor(recordingSeconds / 60).toString().padStart(2, "0");
    $: recordingSecondsDisplay = (recordingSeconds % 60).toString().padStart(2, "0");
    $: recordingTimeLabel = `${recordingMinutes}:${recordingSecondsDisplay}`;
        ...
        try {
            ...
            mediaRecorder.onstop = async () => {
                ...
                if (audioBlob.size > 0) {
                    ...
                    dispatch("sendMessage", { content: "", files: [audioFile], voiceMessage: true, duration: recordingSeconds });
                }
...
<div class="chat-input-wrapper">
    {#if isRecording}
        <div class="recording-bar">
            ...
            <button type="button" class="cancel-recording" on:click={cancelVoiceRecording}>Cancel</button>
        </div>
    {/if}
    ...
    {#if selectedFiles.length > 0 || uploadingFiles.length > 0}
        <div class="file-preview">
            {#each selectedFiles as file, i}
                <div class="file-chip"><span class="file-icon">{file.type.startsWith("audio/")? "🎤" : file.type.startsWith("image/")? "🖼️" : "📎"}</span><span class="file-name">{file.name}</span><button type="button" class="remove-file" on:click={() => removeFile(i)}>×</button></div>
            {/each}
    ...
    {#if showEmojiPicker &&!isRecording}
        <div class="emoji-picker">
            <div class="emoji-header"><span>Emoji</span><button class="emoji-close" on:click={() => showEmojiPicker = false}>×</button></div>
            <div class="emoji-grid">{#each emojis as emoji}<button class="emoji-item" on:click={() => insertEmoji(emoji)}>{emoji}</button>{/each}</div>
        </div>
    {/if}

    <div class="chat-input">
        <button type="button" class="icon-btn" title="Template" disabled={sending || isRecording} on:click={chooseTemplate}>📋</button>
        <button type="button" class:active={showEmojiPicker} class="icon-btn" title="Emoji" disabled={sending || isRecording} on:click={chooseEmoji}>😊</button>
        <button type="button" class="icon-btn" title="Attachment" disabled={sending || isRecording} on:click={chooseAttachment}>📎</button>
        <input bind:this={fileInput} type="file" multiple hidden on:change={handleFileSelect} accept="image/*,video/*,audio/*,.pdf,.doc,.docx,.txt" />
        <textarea bind:value={text} rows="1" placeholder={isRecording? "Recording..." : "Type a message"} disabled={sending || isRecording} on:keydown={keyDown}></textarea>

        {#if text.trim() || selectedFiles.length > 0}
            <button type="button" class="send-btn" on:click={send} disabled={sending || isRecording}>➤</button>
        {:else}
            <button type="button" class:recording={isRecording} class="icon-btn voice" on:click={startVoice} disabled={sending}>{#if isRecording}■{:else}🎤{/if}</button>
        {/if}
    </div>
</div>
