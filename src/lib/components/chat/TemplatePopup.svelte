<script lang="ts">
    ...
    function handleNew() {
        close();
        // ✅ 50K FIX: was localhost:1420 - fails on Vercel
        if(browser){
    ...
    async function handleDelete(template: any) {
        ...
        try {
            ...
            if (!contentType.includes("application/json")) throw new Error(`API returned non-JSON (HTTP ${response.status}).`);
            let result: any;
            try { result = JSON.parse(raw); } catch { throw new Error("Invalid JSON response."); }
            if (!response.ok || result?.success === false) throw new Error(result?.error || `Failed (HTTP ${response.status}).`);
            dispatch("deleted", { template });
        } catch (error) {
            console.error("[TemplatePopup] Delete failed:", error);
            dispatch("error", { error: error instanceof Error? error.message : "Failed to delete." });
    ...
    <section class="popup" role="dialog" aria-modal="true" aria-labelledby="template-title">
        ...
        <div class="toolbar">
            <div class="search-box">
                ...
                <input bind:value={search} type="search" placeholder="Search by name, code..." aria-label="Search templates" />
                {#if search}<button type="button" class="clear-search" aria-label="Clear search" on:click={() => (search = "")}>×</button>{/if}
            </div>
        ...
        <footer class="popup-footer">
            ...
            <span> • <a href="/templates/create">Create in Designer →</a></span>
        </footer>
    </section>
</div>
...
<style>
   ...
   .new-btn { height: 48px; min-width: 108px; padding: 0 18px; border: 0; border-radius: 11px; background: #16a34a; color: #fff; font-size: 15px; font-weight: 700; cursor: pointer; box-shadow: 0 5px 15px rgba(22, 163, 74, 0.22); }
   ...
   .template-card { display: flex; align-items: center; gap: 14px; min-height: 104px; margin: 8px 0; padding: 14px; border: 1px solid #e5e7eb; border-radius: 14px; background: #fff; }
   .template-card:hover { border-color: #d4dce8; box-shadow: 0 6px 20px rgba(15, 23, 42, 0.07); }
   ...
   .action { height: 42px; padding: 0 15px; border: 0; border-radius: 9px; font-size: 14px; font-weight: 700; cursor: pointer; }
