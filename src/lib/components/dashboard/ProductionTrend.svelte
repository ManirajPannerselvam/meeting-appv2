<script lang="ts">
	...
	// FIX 1: Safe date parsing. Your sort was comparing Date objects directly
	...
	// FIX 2: Single pass for all metrics
	$: stats = (() => {
...
<div class="card">
				...
				<div class="column">
					<div class="bars">
						<div
							...
							title={`Target: ${fmt(Number(row.target || 0))}`}
						></div>
						<div
							class="actual"
							style={`height:${percent(Number(row.actual || 0))}%`}
							title={`Actual: ${fmt(Number(row.actual || 0))}`}
						></div>
					</div>
	...
	<div class="summary">
		<div>
			...
			<strong>{fmt(stats.totalTarget)}</strong>
		</div>
		<div>
			<label>Total Actual</label>
			<strong>{fmt(stats.totalActual)}</strong>
		</div>
	<div>
			<label>Good Qty</label>
			<strong>{fmt(stats.totalGood)}</strong>
		</div>
		<div>
			<label>NG Qty</label>
			<strong>{fmt(stats.totalNG)}</strong>
		</div>
		<div>
...
.legend .target { background: #94a3b8; }
...
.target { background: #94a3b8; }
.actual { background: #2563eb; }
